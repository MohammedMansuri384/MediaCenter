...
void GLCD_FillRect (unsigned int x, unsigned int y, unsigned int w, unsigned int h, unsigned short color) {
  unsigned long i;
  
  // Calculate total pixels. Use long to prevent overflow.
  unsigned long num_pixels = (unsigned long)w * h;

  // Set the window to draw in
  GLCD_SetWindow(x, y, w, h);

  // Send the command to write to GRAM
  wr_cmd(0x22); 
  
  // Start the data write
  wr_dat_start();

  // Loop and send the same color for every pixel
