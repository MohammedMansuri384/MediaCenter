...
void get_potval (void) { 
  ...
  PotVal = ((val >> 8) & 0xF8) +          /* Extract Potenciometer Value */
    ...
    if (PotVal > 252) num_leds = 8;
    else if (PotVal > 216) num_leds = 7;
    else if (PotVal > 180) num_leds = 6;
    else if (PotVal > 144) num_leds = 5;
    else if (PotVal > 108) num_leds = 4;
    else if (PotVal > 72)  num_leds = 3;
    else if (PotVal > 36)  num_leds = 2;
    else if (PotVal > 0)   num_leds = 1;

    int i;
    for(i=0; i<8; i++) {
        if(i < num_leds) LED_On(i);
        else LED_Off(i);
    }
}
...
int audio_main (void)
{
  ...
  GLCD_DisplayString(0,5,1, "MP3 File Playing");
  GLCD_DisplayString(2,0,1, "__________________");
